...
exports.executeAdministrativeDecisionExplanationEngine = executeAdministrativeDecisionExplanationEngine;
...
function stableExplanationId(trace) {
    return `EXPLAIN:${trace.moduleId}:${trace.targetType}:${trace.targetId}`;
}
function mapStepTypeToBlockType(stepType) {
    return stepType;
}
function buildBlockDescription(step, elementsById) {
    const ids = asArray(step.supportingElementIds).map(asString).filter((s) => s.length > 0);
    const excerpts = ids
...
function buildExplanationForTrace(trace) {
    const supportingElements = asArray(trace.supportingElements);
    ...
    const steps = asArray(trace.decisionSteps);
    const blocks = steps.map((step) => {
        const blockType = mapStepTypeToBlockType(step.stepType);
        const supportingReferences = asArray(step.supportingElementIds)
            .map(asString)
            .filter((s) => s.length > 0);
        return {
            blockType,
...
function executeAdministrativeDecisionExplanationEngine(traces) {
    const safe = asArray(traces);
