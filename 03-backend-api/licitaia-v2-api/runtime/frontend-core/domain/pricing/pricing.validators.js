...
exports.validatePricingInput = validatePricingInput;
...
function validateRequiredTextField(payload, field, label, items, useBlock) {
    ...
    if (value === undefined || value === null) {
        items.push((0, validation_result_factory_1.createValidationItem)('PRICING_FIELD_MISSING', `${label} é obrigatório`, useBlock ? validation_severity_enum_1.ValidationSeverity.BLOCK : validation_severity_enum_1.ValidationSeverity.ERROR, { field }));
        return;
    }
    if (isEmptyText(value)) {
        items.push((0, validation_result_factory_1.createValidationItem)('PRICING_FIELD_EMPTY', `${label} não pode ser vazio`, useBlock ? validation_severity_enum_1.ValidationSeverity.BLOCK : validation_severity_enum_1.ValidationSeverity.ERROR, { field }));
    }
}
function validateRequiredNumericField(payload, field, label, items) {
    const value = payload[field];
    if (value === undefined || value === null) {
        items.push((0, validation_result_factory_1.createValidationItem)('PRICING_FIELD_MISSING', `${label} é obrigatório`, validation_severity_enum_1.ValidationSeverity.BLOCK, { field }));
        return;
    }
    const num = typeof value === 'number' ? value : Number(value);
    if (Number.isNaN(num)) {
        items.push((0, validation_result_factory_1.createValidationItem)('PRICING_VALUE_INVALID', `${label} deve ser um número válido`, validation_severity_enum_1.ValidationSeverity.BLOCK, { field }));
    }
    else if (num <= 0) {
        items.push((0, validation_result_factory_1.createValidationItem)('PRICING_VALUE_NOT_POSITIVE', `${label} deve ser maior que zero`, validation_severity_enum_1.ValidationSeverity.BLOCK, { field }));
    }
}
/**
 ...
 */
function validatePricingInput(payload) {
    const items = [];
    validateRequiredTextField(payload, 'pricingSourceDescription', 'Descrição da origem/referência de preço', items, BLOCK_ON_MISSING);
    validateRequiredTextField(payload, 'referenceItemsDescription', 'Descrição dos itens de referência', items, BLOCK_ON_MISSING);
    validateRequiredNumericField(payload, 'estimatedUnitValue', 'Valor unitário estimado', items);
    validateRequiredNumericField(payload, 'estimatedTotalValue', 'Valor total estimado', items);
    validateRequiredTextField(payload, 'pricingJustification', 'Justificativa da estimativa', items, BLOCK_ON_MISSING);
    ...
    if (payload.referenceDate === undefined || payload.referenceDate === null) {
        items.push((0, validation_result_factory_1.createValidationItem)('PRICING_FIELD_MISSING', 'Data de referência é obrigatória', validation_severity_enum_1.ValidationSeverity.BLOCK, { field: 'referenceDate' }));
    }
    else if (typeof payload.referenceDate !== 'string' || payload.referenceDate.trim() === '') {
        items.push((0, validation_result_factory_1.createValidationItem)('PRICING_FIELD_EMPTY', 'Data de referência não pode ser vazia', validation_severity_enum_1.ValidationSeverity.BLOCK, { field: 'referenceDate' }));
    }
