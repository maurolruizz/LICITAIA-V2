...
exports.validateTrInput = validateTrInput;
...
function validateRequiredTextField(payload, field, label, items, useBlock) {
    ...
    if (value === undefined || value === null) {
        items.push((0, validation_result_factory_1.createValidationItem)('TR_FIELD_MISSING', `${label} é obrigatório`, useBlock ? validation_severity_enum_1.ValidationSeverity.BLOCK : validation_severity_enum_1.ValidationSeverity.ERROR, { field }));
        return;
    }
    if (isEmptyText(value)) {
        items.push((0, validation_result_factory_1.createValidationItem)('TR_FIELD_EMPTY', `${label} não pode ser vazio`, useBlock ? validation_severity_enum_1.ValidationSeverity.BLOCK : validation_severity_enum_1.ValidationSeverity.ERROR, { field }));
    }
}
/**
 ...
 */
function validateTrInput(payload) {
    const items = [];
    validateRequiredTextField(payload, 'objectDescription', 'Descrição do objeto', items, BLOCK_ON_MISSING);
    validateRequiredTextField(payload, 'contractingPurpose', 'Finalidade da contratação', items, BLOCK_ON_MISSING);
    validateRequiredTextField(payload, 'technicalRequirements', 'Requisitos técnicos', items, BLOCK_ON_MISSING);
    validateRequiredTextField(payload, 'executionConditions', 'Condições de execução', items, BLOCK_ON_MISSING);
    validateRequiredTextField(payload, 'acceptanceCriteria', 'Critérios de aceitação', items, BLOCK_ON_MISSING);
    validateRequiredTextField(payload, 'requestingDepartment', 'Área solicitante', items, BLOCK_ON_MISSING);
    validateRequiredTextField(payload, 'responsibleAuthor', 'Responsável pelo TR', items, BLOCK_ON_MISSING);
    if (payload.referenceDate === undefined || payload.referenceDate === null) {
        items.push((0, validation_result_factory_1.createValidationItem)('TR_FIELD_MISSING', 'Data de referência é obrigatória', validation_severity_enum_1.ValidationSeverity.BLOCK, { field: 'referenceDate' }));
    }
    else if (typeof payload.referenceDate !== 'string' || payload.referenceDate.trim() === '') {
        items.push((0, validation_result_factory_1.createValidationItem)('TR_FIELD_EMPTY', 'Data de referência não pode ser vazia', validation_severity_enum_1.ValidationSeverity.BLOCK, { field: 'referenceDate' }));
    }
