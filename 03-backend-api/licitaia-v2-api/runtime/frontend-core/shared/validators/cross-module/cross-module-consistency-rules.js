...
exports.MODULE_DESCRIPTION_KEYS = void 0;
exports.extractDescriptionFromPayload = extractDescriptionFromPayload;
exports.normalizeTokenForComparison = normalizeTokenForComparison;
exports.tokenizeForComparison = tokenizeForComparison;
exports.hasMinimumTermOverlap = hasMinimumTermOverlap;
exports.getCrossValidationRuleId = getCrossValidationRuleId;
exports.applyConsistencyRule = applyConsistencyRule;
...
exports.MODULE_DESCRIPTION_KEYS = {
 ...
 */
function extractDescriptionFromPayload(payload, moduleId) {
    const raw = payload ?? {};
    if (typeof raw !== 'object')
        return '';
    const config = exports.MODULE_DESCRIPTION_KEYS[moduleId];
    const parts = [];
...
function normalizeTokenForComparison(token) {
    ...
    if (t.endsWith('es') && t.length >= 5) {
        ...
        if (base.length >= MIN_TOKEN_LENGTH)
            return base;
    }
...
function tokenizeForComparison(text) {
    ...
    return normalized
        .split(/\s+/)
        .filter((s) => s.length >= MIN_TOKEN_LENGTH && !TRIVIAL_WORDS.has(s));
}
/**
 ...
 */
function getNormalizedTokenSet(tokens) {
    const set = new Set();
    for (const t of tokens) {
        const n = normalizeTokenForComparison(t);
        if (n.length >= MIN_TOKEN_LENGTH && !TRIVIAL_WORDS.has(n))
            set.add(n);
    }
    return set;
}
 ...
 */
function hasMinimumTermOverlap(textPrevious, textCurrent) {
    ...
    for (const t of tokensCurr) {
        const n = normalizeTokenForComparison(t);
        if (n.length >= MIN_TOKEN_LENGTH && !TRIVIAL_WORDS.has(n) && setPrev.has(n))
            return true;
    }
    return false;
}
 ...
 */
function getCrossValidationRuleId(pair) {
...
function applyConsistencyRule(pair, previousDescription, currentDescription, previousModuleId, currentModuleId) {
    ...
    const items = [];
    const prevLabel = exports.MODULE_DESCRIPTION_KEYS[previousModuleId].label;
    const currLabel = exports.MODULE_DESCRIPTION_KEYS[currentModuleId].label;
    ...
    if (prevEmpty && currEmpty) {
        items.push((0, validation_result_factory_1.createValidationItem)(`${ruleId}_BOTH_EMPTY`, `Módulos ${previousModuleId} e ${currentModuleId} sem descrição principal. Verificar preenchimento.`, validation_severity_enum_1.ValidationSeverity.INFO, { details: { pair } }));
        return items;
    }
