...
exports.createInitialOperationalState = createInitialOperationalState;
exports.createRenderToken = createRenderToken;
...
function buildStepFields(step) {
    return (0, flow_step_definitions_1.getRequiredFieldsForStep)(step).map((fieldId) => ({
        fieldId,
        value: null,
        isValid: null,
...
function createMetadataHash(event) {
    return createSha256(event);
}
function createInitialOperationalState(processId) {
    ...
    const state = {
        ...
        revision: 1,
        generatedAt: now,
        stepOrder: [...flow_controller_types_1.FLOW_STEP_ORDER],
        currentStep: 'INIT',
        stepStatusMap: {
            INIT: 'IN_PROGRESS',
            CONTEXT: 'LOCKED',
        ...
        currentStepForm: {
            ...
            fields: initFields.map((state) => ({
                spec: (0, flow_step_definitions_1.getStepFieldSpec)(state.fieldId),
                state,
            })),
        },
        renderToken: '',
...
function createRenderToken(state) {
    ...
    const reviewResultCanonical = state.reviewResult.phase === 'PRE_REVIEW'
        ? { phase: 'PRE_REVIEW' }
        : {
            phase: 'POST_REVIEW',
