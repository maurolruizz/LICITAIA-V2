...
## 5. Limitação documentada (não é falha)

**Prova de execução end-to-end requer PostgreSQL:**
PostgreSQL não está instalado no ambiente de desenvolvimento atual.
O código está correto e TypeScript compila sem erros.
A prova de execução real (9 casos de validação) deve ser realizada com PostgreSQL 14+
configurado conforme `.env.example` e após execução das migrations + seed 001.
