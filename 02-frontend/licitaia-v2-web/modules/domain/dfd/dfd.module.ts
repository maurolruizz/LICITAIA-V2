...
export async function executeDfdModule(input: ModuleInputContract): Promise<ModuleOutputContract> {
  ...
  if (validation.hasBlocking) {
    ...
    if (calculationMemoryCodes.length > 0) {
      events.push(buildCalculationMemoryInvalidEvent(ModuleId.DFD, { invalidCodes: calculationMemoryCodes, calculationMemoryCount: extractedCalculationMemory.count }, processId));
    }
    ...
    if (administrativeJustificationCodes.length > 0) {
      events.push(buildAdministrativeJustificationInvalidEvent(ModuleId.DFD, { invalidCodes: administrativeJustificationCodes, totalJustifications: extractedAdministrativeJustification.count }, processId));
    }
    ...
    if (administrativeNeedCodes.length > 0) {
      events.push(buildAdministrativeNeedInvalidEvent(ModuleId.DFD, { invalidCodes: administrativeNeedCodes, totalNeeds: extractedAdministrativeNeed.count, issueTypes: [...new Set(administrativeNeedCodes)] }, processId));
    }
    ...
    if (procurementStrategyCodes.length > 0) {
      events.push(buildProcurementStrategyInvalidEvent(ModuleId.DFD, { totalStrategies: extractedProcurementStrategy.count, issueTypes: [...new Set(procurementStrategyCodes)] }, processId));
    }
