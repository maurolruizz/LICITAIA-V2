...
import type { ExtractedAdministrativeJustification, AdministrativeJustificationEntry } from './administrative-justification.types';
import {
  ADMINISTRATIVE_COHERENCE_ISSUE_TYPES,
  type AdministrativeCoherenceResult,
  type AdministrativeCoherenceIssue,
  type AdministrativeCoherenceTargetType,
  type AdministrativeCoherenceSeverity,
} from './administrative-coherence.types';

const SEVERITY_BLOCK: AdministrativeCoherenceSeverity = 'BLOCK';
...
function getJustificationText(entry: AdministrativeJustificationEntry): string {
  const parts = [
...
function addIssue(
  ...
  type: AdministrativeCoherenceIssue['type'],
  targetType: AdministrativeCoherenceTargetType,
  targetId: string,
  message: string,
  severity: AdministrativeCoherenceSeverity = SEVERITY_BLOCK
): void {
 ...
 */
export function executeAdministrativeCoherenceEngine(
  extractedStructure: ExtractedProcurementStructure,
  extractedCalculationMemory: ExtractedCalculationMemory,
  extractedAdministrativeJustification: ExtractedAdministrativeJustification
): AdministrativeCoherenceResult {
  const issues: AdministrativeCoherenceIssue[] = [];
  ...
  for (const entry of extractedAdministrativeJustification.entries) {
    if (entry.targetType !== 'item' && entry.targetType !== 'lot') continue;
    ...
    if (!exists && (validItemIds.size > 0 || validLotIds.size > 0)) {
      addIssue(
        issues,
        ADMINISTRATIVE_COHERENCE_ISSUE_TYPES.JUSTIFICATION_TARGET_NOT_FOUND,
        entry.targetType as AdministrativeCoherenceTargetType,
        targetId,
        `Justificativa administrativa referencia ${entry.targetType} inexistente: ${targetId}.`
      );
  ...
  const justificationItemIds = new Set<string>();
  const justificationLotIds = new Set<string>();
  for (const entry of extractedAdministrativeJustification.entries) {
    if (entry.targetType === 'item' && entry.targetId) justificationItemIds.add(getText(entry.targetId));
    if (entry.targetType === 'lot' && entry.targetId) justificationLotIds.add(getText(entry.targetId));
  }
  if (extractedStructure.structureType === 'multiple_items' && extractedStructure.structure.items) {
    for (const item of extractedStructure.structure.items) {
      if (!justificationItemIds.has(item.id)) {
        addIssue(
          issues,
          ADMINISTRATIVE_COHERENCE_ISSUE_TYPES.OBJECT_WITHOUT_JUSTIFICATION,
          'item',
          item.id,
          `Item "${item.id}" não possui justificativa administrativa associada.`
        );
    ...
    for (const lot of extractedStructure.structure.lots) {
      if (!justificationLotIds.has(lot.id)) {
        addIssue(
          ...
          'lot',
          lot.id,
          `Lote "${lot.id}" não possui justificativa administrativa associada.`
        );
        objectWithoutJustificationCount++;
      }
    }
  ...
  for (const calc of extractedCalculationMemory.entries) {
    ...
    const hasJustification =
      calc.targetType === 'ITEM'
        ? justificationItemIds.has(targetId)
        : justificationLotIds.has(targetId);
    if (!hasJustification) {
      addIssue(
        issues,
        ADMINISTRATIVE_COHERENCE_ISSUE_TYPES.CALCULATION_WITHOUT_JUSTIFICATION,
        calc.targetType === 'ITEM' ? 'item' : 'lot',
        targetId,
        `Memória de cálculo para ${calc.targetType} "${targetId}" sem justificativa administrativa correspondente.`
      );
  ...
  const justificationByTarget = new Map<string, AdministrativeJustificationEntry[]>();
  for (const entry of extractedAdministrativeJustification.entries) {
    if (entry.targetType !== 'item' && entry.targetType !== 'lot') continue;
    const key = `${entry.targetType}:${getText(entry.targetId)}`;
    if (!justificationByTarget.has(key)) justificationByTarget.set(key, []);
  ...
  for (const calc of extractedCalculationMemory.entries) {
    ...
    for (const j of justifications) {
      ...
      const mismatch =
        (suggestsConsumption && calc.calculationType === 'INSTITUTIONAL_SIZING') ||
        (suggestsSizing && calc.calculationType === 'CONSUMPTION');
      if (mismatch) {
        addIssue(
          issues,
          ADMINISTRATIVE_COHERENCE_ISSUE_TYPES.JUSTIFICATION_CALCULATION_MISMATCH,
          targetTypeKey as AdministrativeCoherenceTargetType,
          targetId,
          `Incompatibilidade: justificativa sugere ${suggestsConsumption ? 'consumo/demanda histórica' : 'dimensionamento institucional'}, mas memória de cálculo é ${calc.calculationType}.`
        );
