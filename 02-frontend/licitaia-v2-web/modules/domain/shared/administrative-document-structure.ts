...
import type { DocumentSectionType, DocumentSourceType } from './administrative-document.types';
...
export interface DocumentSectionRule {
  blockId: string;
  ...
  getApplicability(ctx: DocumentClassificationContext): 'required' | 'conditional' | 'prohibited' | 'not_applicable';
}

type RuleSet = Record<DocumentSectionType, DocumentSectionRule>;

function alwaysRequired(): 'required' {
  return 'required';
}

function requiredWithCalculation(ctx: DocumentClassificationContext): 'required' | 'not_applicable' {
  return ctx.hasCalculationData ? 'required' : 'not_applicable';
}

function requiredWithPricing(ctx: DocumentClassificationContext): 'required' | 'not_applicable' {
  return ctx.hasPricingData ? 'required' : 'not_applicable';
}

function calculationRequiredOrProhibitedInSingleExecution(
  ctx: DocumentClassificationContext
): 'required' | 'prohibited' | 'not_applicable' {
  if (ctx.hasCalculationData) return 'required';
  if (ctx.executionForm === 'ENTREGA_UNICA') return 'prohibited';
  return 'not_applicable';
}

function pricingRequiredOrProhibited(
  ctx: DocumentClassificationContext
): 'required' | 'prohibited' {
...
export const DOCUMENT_STRUCTURE_RULES: Record<ModuleId, RuleSet> = {
  'DFD': {
    IDENTIFICATION: {
