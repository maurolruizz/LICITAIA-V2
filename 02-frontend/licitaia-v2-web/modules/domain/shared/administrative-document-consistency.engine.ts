...
import type {
  ...
  AdministrativeDocumentConsistencyIssueType,
} from './administrative-document-consistency.types';
import {
  ADMINISTRATIVE_DOCUMENT_CONSISTENCY_ISSUE_TYPES,
  NEED_RECURRING_KEYWORDS,
  NEED_CONSUMPTION_KEYWORDS,
  NEED_SIZING_KEYWORDS,
  LEGAL_BASIS_REQUIRED_MODALITIES,
  LEGAL_BASIS_REQUIRED_KEYWORDS,
} from './administrative-document-consistency.types';
...
function needSuggestsConsumption(needText: string): boolean {
  return NEED_CONSUMPTION_KEYWORDS.some((t) => needText.includes(t));
}

/** Need menciona dimensionamento institucional (compatível com INSTITUTIONAL_SIZING). Usa NEED_SIZING_KEYWORDS. */
function needSuggestsSizing(needText: string): boolean {
  return NEED_SIZING_KEYWORDS.some((t) => needText.includes(t));
}

/** Texto sugere necessidade recorrente ou previsível. Usa NEED_RECURRING_KEYWORDS. */
function textSuggestsRecurring(text: string): boolean {
  return NEED_RECURRING_KEYWORDS.some((t) => text.includes(t));
}

/** Justificativa menciona base legal para dispensa/inexigibilidade. Usa LEGAL_BASIS_REQUIRED_KEYWORDS. */
function justificationMentionsDirectProcurement(text: string): boolean {
  return LEGAL_BASIS_REQUIRED_KEYWORDS.some((t) => text.includes(t));
}

/** Strategy é modalidade que exige base legal. Usa LEGAL_BASIS_REQUIRED_MODALITIES. */
function isDirectModality(modality: string): boolean {
  const m = modality.toUpperCase().trim();
  return (LEGAL_BASIS_REQUIRED_MODALITIES as readonly string[]).includes(m);
}
 ...
 */
export function executeAdministrativeDocumentConsistencyEngine(
  structure: ExtractedProcurementStructure,
  ...
  const issues: AdministrativeDocumentConsistencyIssue[] = [];
  ...
  for (let cIdx = 0; cIdx < calculationMemory.entries.length; cIdx++) {
    ...
    const needForTarget = administrativeNeed.entries.find(
      (n) => (n.targetType === 'item' && n.targetId === calc.targetId) || (n.targetType === 'lot' && n.targetId === calc.targetId)
    );
    if (!needForTarget) continue;
    const needText = [
    ...
    if (calc.calculationType === 'INSTITUTIONAL_SIZING' && needConsumption && !needSizing) {
      issues.push({
        issueType: ADMINISTRATIVE_DOCUMENT_CONSISTENCY_ISSUE_TYPES.CALCULATION_NEED_MISMATCH,
        severity: 'BLOCK',
  ...
  if (structure.structureType === 'single_item' && (strategyLots || strategyMultiple)) {
    issues.push({
      issueType: ADMINISTRATIVE_DOCUMENT_CONSISTENCY_ISSUE_TYPES.STRATEGY_STRUCTURE_MISMATCH,
      severity: 'BLOCK',
  ...
  if (isDirectModality(modality ?? '')) {
    ...
    if (textSuggestsRecurring(allNeedText)) {
      issues.push({
        issueType: ADMINISTRATIVE_DOCUMENT_CONSISTENCY_ISSUE_TYPES.STRATEGY_NEED_MISMATCH,
        severity: 'WARNING',
  ...
  for (let jIdx = 0; jIdx < administrativeJustification.entries.length; jIdx++) {
    ...
    if (j.targetType !== 'item' && j.targetType !== 'lot' || !j.targetId) continue;
    const needForTarget = administrativeNeed.entries.find(
      (n) => (n.targetType === 'item' && n.targetId === j.targetId) || (n.targetType === 'lot' && n.targetId === j.targetId)
    );
    if (!needForTarget) continue;
    ...
    if (needText.length >= 20 && justText.length >= 20) {
      ...
      if (!overlap && needWords.length >= 3) {
        issues.push({
          issueType: ADMINISTRATIVE_DOCUMENT_CONSISTENCY_ISSUE_TYPES.JUSTIFICATION_NEED_MISMATCH,
          severity: 'WARNING',
  ...
  if (modality && isDirectModality(modality)) {
    ...
    if (justTexts.length >= 10 && !justificationMentionsDirectProcurement(justTexts)) {
      issues.push({
        issueType: ADMINISTRATIVE_DOCUMENT_CONSISTENCY_ISSUE_TYPES.JUSTIFICATION_STRATEGY_MISMATCH,
        severity: 'WARNING',
  ...
  const issueTypes = [...new Set(issues.map((i) => i.issueType))] as AdministrativeDocumentConsistencyIssueType[];
