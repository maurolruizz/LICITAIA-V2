...
import type {
  AdministrativeJustificationEntry,
  AdministrativeJustificationTargetType,
  ExtractedAdministrativeJustification,
} from './administrative-justification.types';
...
function normalizeTargetType(value: unknown): AdministrativeJustificationTargetType | null {
...
function hasMaterialContent(...texts: string[]): boolean {
 ...
 */
function normalizeEntry(raw: unknown): AdministrativeJustificationEntry | null {
  const rec = asRecord(raw);
  if (!rec) return null;

  const targetTypeNormalized = normalizeTargetType(rec.targetType);
  const targetType: AdministrativeJustificationEntry['targetType'] =
  ...
  const extractedFrom =
    extractedFromRaw && extractedFromRaw.length > 0
 ...
 */
export function extractAdministrativeJustification(
  ...
  const entries: AdministrativeJustificationEntry[] = [];
  ...
  const missingCriticalFieldsCount = entries.filter(
    (e) =>
      !hasMaterialContent(
        e.problemStatement ?? '',
        e.administrativeNeed ?? '',
        e.expectedOutcome ?? ''
      )
  ).length;
