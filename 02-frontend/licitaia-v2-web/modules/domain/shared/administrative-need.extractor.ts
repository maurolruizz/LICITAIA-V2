...
import type {
  AdministrativeNeedEntry,
  AdministrativeNeedTargetType,
  ExtractedAdministrativeNeed,
} from './administrative-need.types';
...
function normalizeTargetType(value: unknown): AdministrativeNeedTargetType | null {
 ...
 */
function normalizeEntry(raw: unknown): AdministrativeNeedEntry | null {
  const rec = asRecord(raw);
  if (!rec) return null;

  const targetTypeNormalized = normalizeTargetType(rec.targetType);
  const targetType: AdministrativeNeedEntry['targetType'] =
 ...
 */
export function extractAdministrativeNeed(
  ...
  const entries: AdministrativeNeedEntry[] = [];
  for (let i = 0; i < candidates.length; i++) {
    const e = normalizeEntry(candidates[i]);
    if (!e) continue;
    const fromArray = i < listLength;
    const origin: AdministrativeNeedEntry['origin'] = fromArray
  ...
  if (entries.length === 0) {
    const extractedJustification = extractAdministrativeJustification(raw);
    if (extractedJustification.entries.length > 0) fallbackApplied = true;
    for (let jIndex = 0; jIndex < extractedJustification.entries.length; jIndex++) {
      const j = extractedJustification.entries[jIndex]!;
      if (j.targetType !== 'process' && j.targetType !== 'item' && j.targetType !== 'lot') continue;
      const derivedFrom =
        Array.isArray(raw['administrativeJustifications'])
          ? ('administrativeJustifications' as const)
          : ('administrativeJustification' as const);
      entries.push({
  ...
  const needWithoutProblemCount = entries.filter(
    (e) => getText(e.problemDescription).length < MIN_MATERIAL_LENGTH
  ).length;
  const needWithoutOutcomeCount = entries.filter(
    (e) => getText(e.expectedOutcome).length < MIN_MATERIAL_LENGTH
  ).length;
